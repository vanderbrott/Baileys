	...
	switch (event) {
			...
			for (const update of eventData as ChatUpdate[]) {
				...
				if (conditionMatches) {
					...
					const upsert =
						...
						data.chatUpserts[chatId]
					if (upsert) {
						concatChats(upsert, update)
					} else {
						// merge the update into the existing update
						const chatUpdate = data.chatUpdates[chatId] || {}
						data.chatUpdates[chatId] = concatChats(
							chatUpdate,
							update
						)
					}
		...
		case 'contacts.update':
			...
			for (const update of contactUpdates) {
				...
				const upsert =
					data.historySets.contacts[id] || data.contactUpserts[id]
				if (upsert) {
					Object.assign(upsert, update)
				} else {
					// merge into prior update
					const contactUpdate = data.contactUpdates[id] || {}
					data.contactUpdates[id] = Object.assign(
						contactUpdate,
						update
					)
				}
		...
		case 'messages.reaction':
			...
			for (const { key, reaction } of reactions) {
				...
				if (existing) {
					...
					data.messageReactions[keyStr] = data.messageReactions[
						keyStr
					] || {
						key,
						reactions: [],
		...
		case 'message-receipt.update':
			const receipts =
				eventData as BaileysEventMap['message-receipt.update']
			for (const { key, receipt } of receipts) {
				...
				if (existing) {
					...
					data.messageReceipts[keyStr] = data.messageReceipts[
						keyStr
					] || {
						key,
						userReceipt: [],
